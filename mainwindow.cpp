    ...
    ui(new Ui::MainWindow)
{
    ...
    skyCoverageWidget = new SkyCovWidget(selectedStationModel);
    QVBoxLayout *vbox_skycov = new QVBoxLayout;
    vbox_skycov->addWidget(skyCoverageWidget,1);
    ui->groupBox_sky_coverage->setLayout(vbox_skycov);
    connect(selectedStationModel, SIGNAL(itemChanged(QStandardItem *)), skyCoverageWidget, SLOT(addStations()));
...
//        ui->DoubleSpinBox_axis2up->setValue(*ax2up);
    ...
    if(influenceDistance.is_initialized()){
        skyCoverageWidget->setDistance(*influenceDistance);
//        ui->influenceDistanceDoubleSpinBox->setValue(*influenceDistance);
    ...
    if(influenceInterval.is_initialized()){
        skyCoverageWidget->setTime(*influenceInterval);
//        ui->influenceTimeSpinBox->setValue(*influenceInterval);
    ...
    if(distanceType.is_initialized()){
        skyCoverageWidget->setDistanceType(QString::fromStdString(*distanceType));
//        ui->comboBox_skyCoverageDistanceType->setCurrentText(QString::fromStdString(*distanceType));
    ...
    if(timeType.is_initialized()){
        skyCoverageWidget->setTimeType(QString::fromStdString(*timeType));
...
void MainWindow::on_pushButton_outputSnrTable_2_clicked()
{
    if(parsedSchedule.is_initialized()){
        QString startPath = ui->lineEdit_sessionPath->text();
        QDir dir = QFileInfo(startPath).dir();

        QString path = QFileDialog::getExistingDirectory(this, "Browse to folder", dir.canonicalPath());
        if( !path.isEmpty() ){
            if(path[path.size()-1] != '/'){
                path += '/';
            }
...
{
    if (arg1 == 1){
        ui->doubleSpinBox_efficiencyFactor->setValue(0.57);
    }else{
        ui->doubleSpinBox_efficiencyFactor->setValue(0.64);
    }
}
